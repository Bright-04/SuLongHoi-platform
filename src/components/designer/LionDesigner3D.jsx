...
import lanSuHeadModel from "../../assets/models/lan-su-head.glb";
...
const LionHead = ({ onModelLoad, onModelError }) => {
		...
		if (modelScene) {
			...
			modelScene.traverse((child) => {
				if (child.isMesh) {
					...
					if ((meshName.includes("object_15") || meshName.includes("object_16")) && materialName.includes("material") && !materialName.includes("material_")) {
	...
	return (
		<group ref={meshRef} scale={design.scale} position={[design.position.x, design.position.y, design.position.z]}>
			...
			{!modelError && <ModelWithSuspense modelPath={lanSuHeadModel} onLoad={handleModelLoad} onError={handleModelError} />}
...
const LionDesigner3D = () => {
	...
	return (
		<div className="lion-3d-viewer" style={{ height: "100%", minHeight: "500px" }}>
			<Canvas ref={canvasRef} camera={{ position: [3, 2, 4], fov: 45 }} onCreated={() => setIsLoading(false)} style={{ height: "100%" }}>
				{/* Lighting */}
				...
				{/* Lion Head */}
				<LionHead onModelLoad={setModelLoaded} onModelError={(loaded) => setModelLoaded(loaded)} />

				{/* Controls */}
				<OrbitControls ref={controlsRef} enablePan={true} enableZoom={true} enableRotate={true} minDistance={2} maxDistance={10} target={[0, 0.5, 0]} />
			</Canvas>
